...
#res= 0.5
  ...
  #Load libraries and new functions
  
seurat_analysis<-function(counts,project ,processing=TRUE,normalization=TRUE,clustering=FALSE,signature_genes=FALSE,min.cell=3,min.feat=200,species='human',mt=20,n_hvg=2000,dim=100, res=0.5)
{
 ...
 # Input data
   ...
   if(is.character(counts)){
     ...
     seurat<-CreateSeuratObject( x,min.cells = min.cell,min.features =min.feat,project=project )}
  else if(is.matrix(counts) |  is.data.frame(counts)){
    seurat<-CreateSeuratObject(counts=counts,min.cells = min.cell,min.features =min.feat,project =project )
  }
  else{seurat<-counts}
  ...
  if(normalization){  
    ...
    seurat <- FindVariableFeatures(seurat, selection.method = "vst", nfeatures = n_hvg,verbose = FALSE)
  ...
  if( clustering){
    ...
    seurat <- FindClusters(seurat, resolution = res,plot.SNN = FALSE,  save.SNN = TRUE)
